*turdedo는 defcon ctf 2014 quals 문제중 하나이다. 
...
### Analysis 
- udp 3544 포트에 알 수 없는 프로토콜을 이용한 서비스가 돌고 있음  
- 프로토콜을 잘 맞추어서 접속하면 shell을 띄울 수 있다.
- shell을 띄우면 간단한 몇 가지 명령을 내릴 수 있는데, 그 중에서 echo 명령에 format strings bug가 있다.
- echo 명령을 실행하기 전에 %n을 filter 해서 %_ 로 모두 변경한다. 

### Key point 
- 프로토콜 맞추어서 shell을 띄우기
- bypass %n filter 
...
```
##### connection info table  0x804e170 (0x54 84 bytes)
...
```
### exploit 
format string 공격을 하기 위해 sprintf의 got를 popen을 call 하기 직전의 주소로 덮어 쓴다. 
